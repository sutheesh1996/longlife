            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $user->height }}</td>
                        <td>{{ $user->weight }}</td>
                        <td>{{ $user->subscription_period }}</td>
                        <td>{{ $user->payment_status == 1 ? 'Active' : 'Inactive' }}</td>
                        <td>{{ $user->device_id ? 'Assigned' : 'Not Assigned' }}</td>
                        <td>
                            <a href="{{ route('users.edit', $user->id) }}" class="btn btn-primary">Edit</a>
                            @if(!$user->device_id)
                            <a href="{{ route('users.assign-device-form', $user->id) }}" class="btn btn-secondary">Assign Device</a>
                            @else
